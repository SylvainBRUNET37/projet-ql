...
describe('UserEquipment.vue', async () => {
  ...
  beforeEach(() => {
    ...
    borrowStoreMock = vi
      .spyOn(borrowStore, 'getUserBorrowedEquipments')
...
=======
  ...
  const router = createRouter({
    history: createWebHistory(),
    routes: [{ path: '/admin/user/:id', name: 'UserDetails', component: UserDetails }],
  })
  ...
  it("affiche les informations de l'utilisateur", async () => {
    ...
    // Vérifier les valeurs dans les champs d'entrée
    expect(userDetailsWrapper.find('input[placeholder="Enter first name"]').element.value).toBe(
      'John',
    )
    expect(userDetailsWrapper.find('input[placeholder="Enter last name"]').element.value).toBe(
      'Doe',
    )
    expect(userDetailsWrapper.find('input[placeholder="Enter email"]').element.value).toBe(
      'john.doe@gmail.com',
    )
    expect(userDetailsWrapper.find('input[placeholder="Enter role"]').element.value).toBe('user')
  })
